                    ...
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <input id="name" type="text"
                                class="w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary @error('name') border-red-500 @enderror"
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mt-2 space-y-2">
                                <div class="flex items-center">
                                    <input id="user_type_client" type="radio"
                                        class="focus:ring-primary h-4 w-4 text-primary border-gray-300"
                                        name="user_type" value="client" {{ old('user_type', 'client') === 'client' ? 'checked' : '' }}>
                                    <label for="user_type_client" class="ml-3 block text-sm font-medium text-gray-700">
                                        {{ __('相談者') }} <span class="text-xs text-gray-500">（タロット鑑定を受けたい方）</span>
                                    </label>
                                </div>
                                <div class="flex items-center">
                                    <input id="user_type_practitioner" type="radio"
                                        class="focus:ring-primary h-4 w-4 text-primary border-gray-300"
                                        name="user_type" value="practitioner" {{ old('user_type') === 'practitioner' ? 'checked' : '' }}>
                                    <label for="user_type_practitioner" class="ml-3 block text-sm font-medium text-gray-700">
                                        {{ __('占い師') }} <span class="text-xs text-gray-500">（タロット鑑定を提供したい方）</span>
                                    </label>
                                </div>
                            </div>
                            @error('user_type')
